...
public class Policeman extends Shorty implements Alive, Serializable {
    ...
    Policeman(String s, int x, int y) {
        super(s, x, y);
        armament = false;
    }
    ...
    void punish(Shorty shorty) {
        ...
        if (this.armament) {
            ...
            if (shorty.getEnergy() > 20) {
                Shorty.Angry fighter = new Shorty.Angry();
                fighter.takeBaton(this);
            }
    ...
    public String toString() {
        if (armament) {
            return "Полицейский " + getName() + " с координатами (" + getAbscissa() + "; " + getOrdinate() + ") имеет дубинку " + policemanBaton;
        } else {
